...
Сценарий: 00.01 Установка значений констант, определение типа приложения
	...
	И Я создаю единицу измерения с кодом "796" если ее нет
...
Сценарий: 00.14 Установка цен номенклатуры
	...
	Если '$$ЭтоЕРПУХ$$' Тогда	
		...
		* Вводим документ за Январь
			...
			И я жду открытия окна 'Прайс-лист на *' в течение 5 секунд
				...
				Если '$$ЭтоУХ$$' Тогда
					...
					Если '$$ВерсияУХ$$ = "3.2"' Тогда
						...
						Тогда таблица "ДвиженияДенежныхСредствУХ" стала равной:
							...
							| '1' | 'Реализация программных продуктов' | ''                        | ''              | 'Безналичная'          | 'ООО "Ганимед"'                      | ''           | 'Ганимед-001 от 01.01.2021' | ''           | ''           | ''           | ''           | ''           | '780 000,00'       | '780 000,00'   |
							| '2' | 'Внедрение программных продуктов'  | ''                        | ''              | 'Безналичная'          | 'ООО "Ганимед"'                      | ''           | 'Ганимед-002 от 01.01.2021' | ''           | ''           | ''           | ''           | ''           | '2 340 000,00'     | '2 340 000,00' |
				Если '$$ЭтоЕРПУХ$$' Тогда
