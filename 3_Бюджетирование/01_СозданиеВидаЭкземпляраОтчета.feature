...
Сценарий: 01.06 Работа в контрукторе отчета - Ввод формул
	...
	* Редактируем формулу через форму источников данных
		* Добавляем операнд "Количество"
			...
			Тогда открылось окно 'ВА - Ручной ввод по всем аналитикам_СтрокаАналитика1 количество (Источник данных) *'
			И я нажимаю на кнопку с именем 'ФормаЗаписатьИЗакрыть'
			И я жду закрытия окна 'ВА - Ручной ввод по всем аналитикам_СтрокаАналитика1 количество (Источник данных) *' в течение 5 секунд
		* Добавляем операнд "Цена"
			...
			И я нажимаю на кнопку с именем 'ФормаЗаписатьИЗакрыть'
			И я жду закрытия окна 'ВА - Ручной ввод по всем аналитикам_СтрокаАналитика1 цена (Источник данных) *' в течение 5 секунд
			Тогда открылось окно 'Источники данных'
	...
	* Проверим, что открывается и записывается форма правила расчета
		...
		И я нажимаю на кнопку с именем 'ФормаЗаписатьИЗакрыть'
		И я жду закрытия окна 'ВА - Ручной ввод по всем аналитикам (Правила расчета)' в течение 5 секунд
		Тогда открылось окно 'Конструктор отчета*'
		И я нажимаю на кнопку открытия поля с именем "ПравилоОбработки"
		Тогда открылось окно 'ВА - Ручной ввод по всем аналитикам (Правила расчета)'
		И Я закрываю окно 'ВА - Ручной ввод по всем аналитикам (Правила расчета)'
...
Сценарий: 01.07 Создание бланка вида отчета
	...
	И Я для вида отчета "ВА - Ручной ввод по всем аналитикам" я создаю многопериодный бланк по умолчанию
...
Сценарий: 01.08 Создание экземпляра отчета
	...
	* Отчет должен быть пустой
		...
		Дано Табличный документ "ПолеТабличногоДокументаМакет" равен макету "3_Бюджетирование\Макеты\ВА_ВидОтчетаСАналитиками_Пустой.mxl"
...
Сценарий: 01.09 Проверяем движения документа
	...
	* Загружаем эталонный макет
		Когда открылось окно 'Экземпляр отчета ВА - Ручной ввод по всем аналитикам:  Валюта отображения:  RUB <Меркурий ООО> <ВА - Основной проект> <Январь * г. - Март * г. (Периодичность: Месяц) <ВА - Основной сценарий>>*'
		И я буду выбирать внешний файл "$КаталогПроекта$\3_Бюджетирование\Макеты\ВА_ВидОтчетаСАналитиками_Меркурий.mxl" 
	...
	* Проверяем движения документа
		Если '$$РасширенныйАлгоритмРасчетаПоказателей$$' Тогда
			...
			И Пауза 5
			Дано Табличный документ 'ОтчетТабличныйДокумент' равен макету "3_Бюджетирование\Макеты\ВА_ВидОтчетаСАналитиками_Меркурий_Движения.mxl" по шаблону
...
Сценарий: 01.10 Копирование экземпляра отчета
	...
	* Загружаем эталонный макет
		Когда открылось окно 'Экземпляр отчета ВА - Ручной ввод по всем аналитикам:  Валюта отображения:  RUB <Венера ООО> <ВА - Основной проект> <Январь * г. - Март * г. (Периодичность: Месяц) <ВА - Основной сценарий>> *'
		Тогда я буду выбирать внешний файл с индексом фильтра 108 "$КаталогПроекта$\3_Бюджетирование\Макеты\ВА_ВидОтчетаСАналитиками_Венера.mxl" 
		И я нажимаю на кнопку с именем 'ФормаЗагрузитьЭталонныйМакет'
		Тогда открылось окно '1С:Предприятие'
		И я нажимаю на кнопку с именем 'Button0'
		И Пауза 5
	...
	* Проверяем движения документа
		Если '$$РасширенныйАлгоритмРасчетаПоказателей$$' Тогда
			...
			Тогда открылось окно 'Плоская таблица значений показателей'
			И Пауза 5
			Дано Табличный документ 'ОтчетТабличныйДокумент' равен макету "3_Бюджетирование\Макеты\ВА_ВидОтчетаСАналитиками_Венера_Движения.mxl" по шаблону
			И Я закрываю окно 'Плоская таблица значений показателей'
...
Сценарий: 01.12 Отображение в дополнительной валюте
	...
	* Сравниваем с эталоном
		Тогда открылось окно 'Экземпляр отчета ВА - Ручной ввод по всем аналитикам:  Валюта отображения:  USD <Венера ООО> <ВА - Основной проект> <Январь * г. - Март * г. (Периодичность: Месяц) <ВА - Основной сценарий>> *'
		Дано Табличный документ "ПолеТабличногоДокументаМакет" равен макету "3_Бюджетирование\Макеты\ВА_ВидОтчетаСАналитиками_Венера_USD"
...
Сценарий: 01.14 Открытие версии
	...
	* Откроем версию
		...
		Тогда открылось окно 'ВА - Ручной ввод по всем аналитикам(многопериодный) (Режим просмотра версии)'
		И я нажимаю на кнопку с именем 'ФормаСформировать'
		Дано Табличный документ "ПолеТабличногоДокументаМакет" равен макету "3_Бюджетирование\Макеты\ВА_ВидОтчетаСАналитиками_Пустой.mxl"	
		И Я закрываю окно 'ВА - Ручной ввод по всем аналитикам(многопериодный) (Режим просмотра версии)'
...
Сценарий: 01.16 Сравнение версий
	...
	* Сравним версии
		...
		Когда открылось окно 'ВА - Ручной ввод по всем аналитикам(многопериодный) (Режим сравнения двух версий)'
		Дано Табличный документ "ПолеТабличногоДокументаМакет" равен макету "3_Бюджетирование\Макеты\ВА_ВидОтчетаСАналитиками_СравнениеВерсий_Различные.mxl"
		И Я закрываю окно 'ВА - Ручной ввод по всем аналитикам(многопериодный) (Режим сравнения двух версий)'
...
Сценарий: 01.17 Сворачивание до версии	
	...
	* Сравниваем с эталонным макетом
		Дано Табличный документ "ПолеТабличногоДокументаМакет" равен макету "3_Бюджетирование\Макеты\ВА_ВидОтчетаСАналитиками_Венера_2.mxl"
...
Сценарий: 01.18 Изменение отчета при выводе в тысячах
	...
	* Сравниваем с эталоном
		Дано Табличный документ "ПолеТабличногоДокументаМакет" равен макету "3_Бюджетирование\Макеты\ВА_ВидОтчетаСАналитиками_Венера_3.mxl"
...
Сценарий: 01.20 Переходим в режим отображения данных
	...
	* Проверяем содержимое
		...
		Дано Табличный документ "ПолеТабличногоДокументаМакет" равен макету "3_Бюджетирование\Макеты\ВА_ВидОтчетаСАналитиками_СводнаяТаблица1.mxl"
	...
	* Тестируем кнопки настроек
		...
		Когда открылось окно 'Сводная таблица: ВА - Ручной ввод по всем аналитикам(cводная таблица)( Основной) *'
		Дано Табличный документ "ПолеТабличногоДокументаМакет" равен макету "3_Бюджетирование\Макеты\ВА_ВидОтчетаСАналитиками_СводнаяТаблица2.mxl"
	...
	* Меняем показатели
		...
		* Отменяем измененное значение
			Если '$$РасширенныйАлгоритмРасчетаПоказателей$$' Тогда Тогда
				Когда открылось окно 'Сводная таблица: ВА - Ручной ввод по всем аналитикам(cводная таблица)( Основной) *'
...
Сценарий: 01.21 Тестируем кнопки пересчета показателей
	...
	* Сравниваем результат
		Тогда открылось окно 'Сводная таблица: ВА - Ручной ввод по всем аналитикам(cводная таблица)( Основной)*'				
		Дано Табличный документ "ПолеТабличногоДокументаМакет" равен макету "3_Бюджетирование\Макеты\ВА_ВидОтчетаСАналитиками_СводнаяТаблица3.mxl" по шаблону		
