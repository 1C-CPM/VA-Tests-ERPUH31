...
Сценарий: 03.01 Создание внешней информационной базы
	...
	И из выпадающего списка с именем "ТипВнешнегоИсточника" я выбираю по строке 'Табличные документы (1С или Microsoft Excel)'
...
Сценарий: 03.03 Создание многопериодного бланка отчета и выгузка в Excel
	...
	* Найдем вид отчета
		...
		И в таблице "Список" я перехожу к строке:
			| 'Наименование'                     |
			| 'ВА - ИмпортExcel(многопериодный)' |
		И в таблице "Список" я активизирую поле с именем "Ссылка"
		И в таблице "Список" я выбираю текущую строку

	* Экспортируем бланк
		Тогда открылось окно 'Бланк ВА - ИмпортExcel(многопериодный) вида отчета: ВА - ИмпортExcel'
	...
	* Сохраняем файл
		...
		И в поле с именем 'ИмяФайлаВыгрузки' я ввожу текст '$КаталогПроекта$\3_Бюджетирование\Макеты\ВА_ИмпортExcel_Шаблон.xlsx'
		...
		Тогда открылось окно 'Бланк ВА - ИмпортExcel(многопериодный) вида отчета: ВА - ИмпортExcel'
		И Я закрываю окно 'Бланк ВА - ИмпортExcel(многопериодный) вида отчета: ВА - ИмпортExcel'
	...
	* Сравниваем файл
		...
		И в поле с именем 'ПутьКДокументу1' я ввожу текст '$КаталогПроекта$\3_Бюджетирование\Макеты\ВА_ИмпортExcel_Шаблон.xlsx'
		И в поле с именем 'ПутьКДокументу2' я ввожу текст '$КаталогПроекта$\3_Бюджетирование\Макеты\ВА_ИмпортExcel_Шаблон.mxl'
...
Сценарий: 03.04 Создание экземпляра отчета - "ВА - ИмпортExcel"	и загрузка из файла
	...
	* Загружаем из шаблона
		...
		И в поле с именем 'ПутьКФайлу' я ввожу текст '$КаталогПроекта$\3_Бюджетирование\Макеты\ВА_ИмпортExcel_Меркурий.xlsx'
		Тогда открылось окно 'Выбор способа заполнения'
		...
		Дано Табличный документ "ПолеТабличногоДокументаМакет" равен макету "3_Бюджетирование\Макеты\ВА_ИмпортExcel_Меркурий.mxl"	
		И я нажимаю на кнопку с именем 'ЗаписатьИЗакрыть'
...
Сценарий: 03.05 Повторная загрузка данных из файла		
	...
	Тогда открылось окно 'Выбор способа заполнения'
	И я перехожу к закладке с именем "ГруппаСтраницаИмпортИзФайла"
	И в поле с именем 'ПутьКФайлу' я ввожу текст '$КаталогПроекта$\3_Бюджетирование\Макеты\ВА_ИмпортExcel_Меркурий_2.xlsx'	
	Тогда открылось окно 'Выбор способа заполнения'
	И я нажимаю на кнопку с именем 'ЗаполнитьОтчет'
	Тогда открылось окно 'Экземпляр отчета ВА - ИмпортExcel:  Валюта отображения:  RUB <Меркурий ООО> <> <Январь * г. - Март * г. (Периодичность: Месяц) <ВА - Основной сценарий>> *'
	Дано Табличный документ "ПолеТабличногоДокументаМакет" равен макету "3_Бюджетирование\Макеты\ВА_ИмпортExcel_Меркурий_2.mxl"	
	И я нажимаю на кнопку с именем 'ЗаписатьИЗакрыть'
	И я жду закрытия окна 'Экземпляр отчета ВА - ИмпортExcel:  Валюта отображения:  RUB <Меркурий ООО> <> <Январь * г. - Март * г. (Периодичность: Месяц) <ВА - Основной сценарий>> *' в течение 20 секунд
