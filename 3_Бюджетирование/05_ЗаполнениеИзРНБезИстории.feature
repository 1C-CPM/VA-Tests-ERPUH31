...
Сценарий: 05.01 Создание вида отчета "ВА - Заполнение из РН (без истории)"
	...
	* Ищем вид отчета с именем "ВА - Заполнение из РН (без истории)"
		...
		И в поле с именем 'Pattern' я ввожу текст 'ВА - Заполнение из РН (без истории)'
		...
		И Пока в таблице "СписокВидовОтчетов" количество строк "больше" 0 Тогда
			...
			Когда открылось окно 'ВА - Заполнение из РН (без истории) (Виды отчетов)'		
			...
			И я жду закрытия окна 'ВА - Заполнение из РН (без истории) (Виды отчетов)' в течение 20 секунд
	...
	* Создаем новый вид отчета
		...
		И в поле с именем 'Наименование' я ввожу текст 'ВА - Заполнение из РН (без истории)'
	...
	* Редактируем структуру отчета
		Тогда открылось окно 'ВА - Заполнение из РН (без истории) (Виды отчетов)'
	...
	* Создаем бланк отчета
		...
		Тогда открылось окно 'ВА - Заполнение из РН (без истории) (Виды отчетов)'
		И я закрываю окно 'ВА - Заполнение из РН (без истории) (Виды отчетов)'
		Тогда открылось окно 'Виды и бланки отчетов'
		И в таблице "Список" я перехожу к строке:
			| 'Наименование'                             |
			| 'ВА - Заполнение из РН (без истории)(многопериодный)' |
		И в таблице "Список" я выбираю текущую строку
		Тогда открылось окно 'Бланк ВА - Заполнение из РН (без истории)(многопериодный) вида отчета: ВА - Заполнение из РН (без истории)'
		...
		* Вводим формулу расчета
			...
			* Заполняем источник получения, проверяем заполнение полей				
				...
				Если '$$ВерсияУХ$$ = "3.1"' Тогда
					И из выпадающего списка с именем "РегистрНакопленияБД" я выбираю по строке 'ФактПоБюджетам'
					Тогда таблица "ТаблицаСоответствия" стала равной:
						| 'Аналитика приемника'    | 'Способ заполнения' | 'Поле источника' | 'Вид аналитики' |
						| 'Аналитика1: Статьи ДДС' | 'Поле источника'    | 'СтатьяБюджета'  | 'Статьи ДДС'    |
				Если '$$ВерсияУХ$$ = "3.2"' Тогда
					И из выпадающего списка с именем "РегистрНакопленияБД" я выбираю по строке 'ДвиженияДенежныхСредствУХ'
					Тогда таблица "ТаблицаСоответствия" стала равной:
						| 'Аналитика приемника'    | 'Способ заполнения' | 'Поле источника'                 | 'Вид аналитики' |
						| 'Аналитика1: Статьи ДДС' | 'Поле источника'    | 'СтатьяДвиженияДенежныхСредств'  | 'Статьи ДДС'    |	
			...
			* Заполняем отборы		
				...
				Если '$$ВерсияУХ$$ = "3.1"' Тогда
					И в таблице "ДеревоПараметровОтбораБД" я перехожу к строке:
						| 'Отбор'           | 'Поле'       |
						| 'Сценарий отчета' | 'Аналитика1' |
					И я нажимаю на кнопку с именем 'ДеревоПараметровОтбораБДУдалить'
					И в таблице "ДеревоПараметровОтбораБД" я перехожу к строке:
						| 'Отбор'         | 'Поле'                 |
						| 'Период отчета' | 'ДокументПланирования' |
					И я нажимаю на кнопку с именем 'ДеревоПараметровОтбораБДУдалить'					
					И в таблице "ДеревоПолейБД" я выбираю текущую строку
					...
					И в таблице "ДеревоПараметровОтбораБД" из выпадающего списка с именем "УточнениеСпособаОпределения" я выбираю точное значение 'Бюджет движения денежных средств'	
				Если '$$ВерсияУХ$$ = "3.2"' Тогда			
					И в таблице "ДеревоПараметровОтбораБД" я перехожу к строке:
						| 'Отбор'         | 'Поле'       |
...
Сценарий: 05.03 Создание экземпляра отчета - "ВА - Заполнение из РН (без истории)" без записи 
	...
	Если 'НЕ $$ЭтоCPMWE$$' Тогда
		...
		* Создаем новый экземпляр отчета
			...
			И из выпадающего списка с именем "ВидОтчета" я выбираю по строке 'ВА - Заполнение из РН (без истории)'
			...
			* Выбираем организацию
			...
			И я жду открытия окна 'Экземпляр отчета ВА - Заполнение из РН (без истории):  Валюта отображения:  RUB <Меркурий ООО> <ВА - Основной проект> <Январь * г. - Март * г. (Периодичность: Месяц) <ВА - Основной сценарий>> *' в течение 20 секунд

		* Документ должен быть пустой
			Когда открылось окно 'Экземпляр отчета ВА - Заполнение из РН (без истории):  Валюта отображения:  RUB <Меркурий ООО> <ВА - Основной проект> <Январь * г. - Март * г. (Периодичность: Месяц) <ВА - Основной сценарий>> *'
		...
		* Закрываем без записи
			Когда открылось окно 'Экземпляр отчета ВА - Заполнение из РН (без истории):  Валюта отображения:  RUB <Меркурий ООО> <ВА - Основной проект> <Январь * г. - Март * г. (Периодичность: Месяц) <ВА - Основной сценарий>> *'
			И Я закрываю окно 'Экземпляр отчета ВА - Заполнение из РН (без истории):  Валюта отображения:  RUB <Меркурий ООО> <ВА - Основной проект> <Январь * г. - Март * г. (Периодичность: Месяц) <ВА - Основной сценарий>> *'
			Тогда открылось окно '1С:Предприятие'
...
Сценарий: 05.04 Создание экземпляра отчета - "ВА - Заполнение из РН (без истории)" с записью 
	...
	Если 'НЕ $$ЭтоCPMWE$$' Тогда
		...
		* Создаем новый экземпляр отчета
			...
			И я нажимаю на кнопку с именем 'ФормаСоздать'
			Тогда открылось окно 'Укажите ключевые реквизиты документа'
			И из выпадающего списка с именем "ВидОтчета" я выбираю по строке 'Заполнение из РН (без истории)'
			И из выпадающего списка с именем "Сценарий" я выбираю по строке 'ВА - Основной сценарий'
			И из выпадающего списка с именем "Периодичность" я выбираю точное значение 'Месяц'
			И я нажимаю на кнопку с именем 'ИзменитьПериод'
			Тогда открылось окно 'Выберите период'
		...
		* Записываем документ, проверяем движения
			...
			И Пауза 5
			Если '$$РасширенныйАлгоритмРасчетаПоказателей$$' Тогда
				Когда открылось окно 'Экземпляр отчета ВА - Заполнение из РН (без истории):  Валюта отображения:  RUB <Меркурий ООО> <ВА - Основной проект> <Январь * г. - Март * г. (Периодичность: Месяц) <ВА - Основной сценарий>>'
	...
	Если 'НЕ $$ЭтоCPMWE$$' Тогда
		...
		* Вводим формулу расчета
			...
			* Заполняем источник получения, проверяем заполнение полей				
				...
				И у элемента с именем 'РесурсРегистраБД' я жду значения 'Сумма' в течение 10 секунд					
				И элемент формы с именем "ВидИтогаБД" отсутствует на форме					
				Тогда таблица "ДеревоПолейБД" стала равной по шаблону:
					| 'Поле'                   |
					| 'Регистратор'            |
					| 'Измерения регистра(14)' |
					| 'Реквизиты регистра(*)'  |
				Тогда таблица "ТаблицаСоответствия" стала равной:
					| 'Аналитика приемника'    | 'Способ заполнения' | 'Поле источника' | 'Вид аналитики' |
					| 'Аналитика1: Статьи ДДС' | 'Поле источника'    | 'СтатьяБюджета'  | 'Статьи ДДС'    |	
...
Сценарий: 05.06 Создаем и рассчитываем сводную таблицу для "ВА - Заполнение из РН (без истории)"
	...
	Если 'НЕ $$ЭтоCPMWE$$' Тогда
		...
		* Создаем бланк сводной таблицы
			...
			И в таблице "Список" я перехожу к строке:
				| 'Наименование'                                         |
				| 'ВА - Заполнение из РН (без истории)(cводная таблица)' |
			И в таблице "Список" я выбираю текущую строку
			* Состав строк
				Когда открылось окно 'Сводная таблица [макет]: ВА - Заполнение из РН (без истории)(cводная таблица)'
			...
			* Состав колонок	
				Тогда открылось окно 'Сводная таблица [макет]: ВА - Заполнение из РН (без истории)(cводная таблица)*'
			...
			* Запись бланка
				...
				И я нажимаю на кнопку с именем 'ФормаЗаписатьИЗакрыть'
				И я жду закрытия окна 'Сводная таблица [макет]: ВА - Заполнение из РН (без истории)(cводная таблица)*' в течение 20 секунд

		* Создаем сводную таблицу
			Когда открылось окно 'Виды и бланки отчетов'
			И я нажимаю на кнопку с именем 'ПерейтиВРежимОтображенияДанных'
			Тогда открылось окно 'Сводная таблица: ВА - Заполнение из РН (без истории)(cводная таблица)*'
			...
			Тогда табличный документ "ПолеТабличногоДокументаМакет" равен:
				| 'ВА - Заполнение из РН (без истории)(cводная таблица)' | 'Январь 2021 г.' | 'Февраль 2021 г.' | 'Март 2021 г.' |
				| 'ВА - Заполнение из РН (без истории)(cводная таблица)' | 'Сумма'          | 'Сумма'           | 'Сумма'        |
		...
		* Рассчитываем по регламенту
			Когда открылось окно 'Сводная таблица: ВА - Заполнение из РН (без истории)(cводная таблица)*'
		...
		* Сохраняем таблицу
			Когда открылось окно 'Сводная таблица: ВА - Заполнение из РН (без истории)(cводная таблица)*'
			И Я закрываю окно 'Сводная таблица: ВА - Заполнение из РН (без истории)(cводная таблица)*'
	...
	Если 'НЕ $$ЭтоCPMWE$$' Тогда
		...
		* Проверяем движения
			Если '$$РасширенныйАлгоритмРасчетаПоказателей$$' Тогда	
				...
				И я нажимаю на кнопку с именем 'ФормаЗаписатьИПродолжить'
				И Пауза 5
				Тогда открылось окно 'Экземпляр отчета ВА - Заполнение из РН (без истории):  Валюта отображения:  RUB <Меркурий ООО> <ВА - Основной проект> <Январь * г. - Март * г. (Периодичность: Месяц) <ВА - Основной сценарий>>'
				И я нажимаю на кнопку с именем 'ФормаОткрытьДвиженияДокументаПлоскаяТаблица'
				И Пауза 5
				Тогда открылось окно 'Плоская таблица значений показателей'
				Тогда табличный документ "ОтчетТабличныйДокумент" равен по шаблону:
