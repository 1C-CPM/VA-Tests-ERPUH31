...
Сценарий: 03.02. [ЗАПРОС КОТИРОВОК] СПЗ-1 на два года
	...
	* Закладка Потребности
		...
		И в таблице 'Список' я перехожу к строке:
			...
			| "10, кг"     | "1"           | "МестоПоставки1" | "Товар1"       | "Королевство Севера [223]" | "Январь 2024 г."     | "Обычный"   | "85 000" | "8 500" |
		И в таблице 'Список' я выбираю текущую строку
...
Сценарий: 03.03. [ЗАПРОС КОТИРОВОК] ЗП-1(лотовая)
	...
	* Создаем процедуру
		...
		И в таблице "Список" я перехожу к строке:
			...
			| '2024 г.'        | 'Королевство Севера [223]' | 'Семь королевств'     | 'Запрос котировок' | 'Приобретение материалов, тест 1' | 'Утвержден' | '85 000,00'       | 'RUB'              | 'Утвержден' |
	...
	* Закладка лоты
		...
		И в таблице "Лоты" я перехожу к строке:
			...
			| '2024 г.'        | 'Лотовая закупка' | 'Королевство Севера [223]' | 'По ценам'     | 'Приобретение материалов, тест 1' | '170 000,00, RUB' |
...
Сценарий: 03.04. [ЗАПРОС КОТИРОВОК] Протокол -> ДД1(ЗП1)
	...
	* Открываем закупочную процедуру
		...
		И в таблице "Список" я перехожу к строке:
			...
			|'Королевство Севера [223]'| 'Семь королевств'     | '2024 г.'        | 'Приобретение материалов, тест 1' | 'Запрос котировок' |
		И в таблице "Список" я выбираю текущую строку
		Тогда открылось окно 'Закупочная процедура №*'
...
//			| "Товар"        |
		...
		* Цена 
		...
		И в таблице 'ПотребностиПоГодам' я активизирую поле с именем 'ПотребностиПоГодамГод2024Сумма'
...
Сценарий: 03.07. [ЗАПРОС ПРЕДЛОЖЕНИЙ] ЗП-2(попозиционная)
	...
	* Создаем процедуру
		...
		И в таблице "Список" я перехожу к строке:
			...
			| 'RUB'              | 'Королевство Севера [223]' | 'Семь королевств'     |'2024 г.'        | 'Приобретение материалов, тест 2' | 'Утвержден' | 'Запрос предложений' | 'Утвержден' | '510 000,00'      |
	...
	* Закладка лоты
		...
		И в таблице "Лоты" я перехожу к строке:
			| 'Период закупок' | 'Вид закупки'           | 'Заказчик'                 | 'Метод оценки' | 'Предмет договора'                | 'НМЦ'             |
			| '2024 г.'        | 'Попозиционная закупка' | 'Королевство Севера [223]' | 'По ценам'     | 'Приобретение материалов, тест 2' | '510 000,00, RUB' |
